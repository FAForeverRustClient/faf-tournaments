 ...
 *
 * `opts` exists for stage-2 playoff brackets, which are built from a subset of the field
 * (the teams that came through the Swiss stage) using a separate seed field, so the
 * original rating-derived seeds stay intact and readable. With no opts the behaviour is
 * exactly what it always was.
 *
 * @param {{ teams: TeamSeed[] }} t
 * @param {number} [division] 0 or undefined = all teams
 * @param {{ only?: string[], seedKey?: string, slots?: string[] }} [opts]
 * @returns {string[]}
 */
function seededSlots(t, division, opts) {
  // An explicit slot list wins outright: that is how a player-chosen first round is built.
  if (opts && Array.isArray(opts.slots) && opts.slots.length) return opts.slots.slice();
  let teams = t.teams.slice();
  if (division && division > 0) teams = teams.filter(x => (x.division || 0) === division);
  const o = opts || {};
  if (Array.isArray(o.only)) {
    const keep = Object.create(null);
    for (const id of o.only) keep[id] = 1;
    teams = teams.filter(x => keep[x.id]);
  }
  const key = (typeof o.seedKey === 'string' && o.seedKey) ? o.seedKey : 'seed';
  teams.sort((a, b) => (a[key] || 0) - (b[key] || 0));
