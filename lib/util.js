...
function json(res, code, obj) {
  ...
  const req = res.req;   // Node exposes the request on the response; avoids changing every call site
  const h = { 'Content-Type': 'application/json; charset=utf-8', 'Cache-Control': 'no-store' };

  // ETag on successful GETs. The client polls a full tournament snapshot every few seconds and
  // throws it away whenever nothing changed; this turns those polls into an empty 304.
  // `no-cache` (not `no-store`) is required for the browser to keep a copy to revalidate against.
  // It still revalidates on every single use, so nothing stale is ever shown, and because the
  // ETag is computed from the response we just built for THIS session, a viewer whose role or
  // login changed simply gets a different tag and a fresh body.
  if (req && req.method === 'GET' && code === 200) {
    const tag = '"' + crypto.createHash('sha1').update(body).digest('base64') + '"';
    h['ETag'] = tag;
    h['Cache-Control'] = 'private, no-cache';
    h['Vary'] = 'Accept-Encoding, Cookie';
    if (req.headers['if-none-match'] === tag) {
      res.writeHead(304, { 'ETag': tag, 'Cache-Control': 'private, no-cache', 'Vary': 'Accept-Encoding, Cookie' });
      return res.end();
    }
  }

  // gzip once the body is big enough to gain from it. Below ~1 KB compression makes it larger.
  // Level 4 keeps almost all of the size win at a fraction of level 6's CPU, which matters on a
  // single-threaded server.
  if (Buffer.byteLength(body) >= 1024 && /\bgzip\b/.test(String((req && req.headers['accept-encoding']) || ''))) {
    let gz = null;
    try { gz = zlib.gzipSync(body, { level: 4 }); } catch (e) { gz = null; }
    if (gz) {
