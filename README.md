...
### Teams and seeding
- Places are **first come, first served**: a team claims its slot when it *fills up*, not when it was created and not by rating. Rating only decides seeding. Beyond the cap, teams queue on a waiting list in the order they completed, and checked-in teams still sort ahead when check-in is in use. The Teams tab and the actual start use one shared rule, so the list can never show something different from what starting does.
